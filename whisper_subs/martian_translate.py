...
def translate_srt(content, tokenizer, model, model_type, target_language):
    ...
    for block in blocks:
        ...
        translated_text = translate_text(
            text_to_translate, tokenizer, model, model_type, target_language
        )
...
def translate_txt(content, tokenizer, model, model_type, target_language):
    translated_text = translate_text(
        content, tokenizer, model, model_type, target_language
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="en",
        help="Source language code (e.g., en, sk)",
    )
    parser.add_argument(
        "--target_language",
        type=str,
        default="sk",
        help="Target language code (e.g., sk, en)",
    )
    parser.add_argument(
        "--translation_model",
        type=str,
    ...
    if model_type == "MarianMT":
        model_name = (
            f"Helsinki-NLP/opus-mt-{args.source_language}-{args.target_language}"
        )
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith((".srt", ".txt")):
            ...
            if filename.lower().endswith(".srt"):
                ...
                translated_content = translate_txt(
                    content,
                    tokenizer,
                    translation_model,
                    model_type,
