...
def compute_excel_diff(df_input, df_output):
    ...
    all_columns = list(df_input.columns) if not df_input.empty else list(df_output.columns)
    ...
    addition_mask = (df_input.fillna('') != df_output.fillna('')) & df_output.fillna('').ne('')
    missing_mask = (df_input.fillna('') != df_output.fillna('')) & df_input.fillna('').ne('')
...
def document_diff(df_input, df_output, addition_mask, missing_mask, file_name, input_file, output_file, summary_file):
    ...
    for idx in addition_mask.index:
        for col in addition_mask.columns:
            if addition_mask.at[idx, col]:
                lines.append(f"  Row {int(idx)+2}, Column {col}: input='{df_input.at[idx, col]}' output='{df_output.at[idx, col]}' (ADDED)")
    ...
    for idx in missing_mask.index:
        for col in missing_mask.columns:
            if missing_mask.at[idx, col]:
                lines.append(f"  Row {int(idx)+2}, Column {col}: input='{df_input.at[idx, col]}' output='{df_output.at[idx, col]}' (MISSING)")
...
def process_excel_pair(input_path, output_path, diff_folder):
    ...
    df_input, df_output, addition_mask, missing_mask = compute_excel_diff(df_input, df_output)
    ...
    summary_file = os.path.join(summary_dir, f"{os.path.splitext(file_name)[0]}_summary.txt")
    document_diff(df_input, df_output, addition_mask, missing_mask, file_name, input_path, output_path, summary_file)
